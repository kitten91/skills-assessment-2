...
def without_duplicates(words):
    ...
    """
    no_dupes = set()
    no_dupes_add = no_dupes.add

    return [x for x in words if not (x in no_dupes or no_dupes_add(x))]
...
def count_unique(input_string):
    ...
    word_dictionary = [wordlist.count(p) for p in wordlist]
    return dict(zip(wordlist, word_dictionary))
...
def translate_to_pirate_talk(phrase):
   ...
   English     Pirate
    ...
    for key in dictionary.items():
          phrase  = dictionary.setdefault(dictionary[key], default)
...
def sort_by_word_length(words):
    ...
    return sort_by_word_length(words)
...
def get_sum_zero_pairs(input_list):
    ...
    """
    newlist = [int*-1 for int in input_list]
    return newlist
